...
namespace GeometryAPI
{
    ...
    public class Circle : Figure, IFlat, IAreaMeasurable
    {
        ...
        {
            Vector3D normal = Vector3D.CrossProduct(new Vector3D(base.vertices[0].X + this.Radius, base.vertices[0].Y, base.vertices[0].Z) - base.vertices[0], new Vector3D(base.vertices[0].X, base.vertices[0].Y + this.Radius, base.vertices[0].Z) - base.vertices[0]);
            normal.Normalize();
            return normal;
        }
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        public double Height
        {
            get
            {
                return new LineSegment(base.vertices[0], base.vertices[1]).GetLength();  
            }
        }
        ...
        public double GetArea()
        {
            double sideHeight = new LineSegment(base.vertices[0], base.vertices[1]).GetLength();
            return Math.PI * Math.Pow(this.Radius, 2) * 2 + 2 * Math.PI * this.Radius * sideHeight;
        }
        ...
        public override Vector3D GetCenter()
        {
            return (base.vertices[0] + base.vertices[1]) / 2;
        }
    }
    ...
    {
        public AdvancedFigureController()
            : base()
        {
        }
                ...
                case "circle":
                    {
                        ...
                        currentFigure = new Circle(centre, radius);
                        break;
                    }
                case "cylinder":
                    {
                        Vector3D topCentre = Vector3D.Parse(splitFigString[1]);
                        Vector3D bottomCentre = Vector3D.Parse(splitFigString[2]);
                        double radius = double.Parse(splitFigString[3]);
                        currentFigure = new Cylinder(topCentre, bottomCentre, radius);
                        break;
                    }
                default:
                    {
                ...
                case "normal":
                    {
                        if (this.currentFigure is IFlat)
                        {
                            ...
                            Console.WriteLine(figure.GetNormal());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
