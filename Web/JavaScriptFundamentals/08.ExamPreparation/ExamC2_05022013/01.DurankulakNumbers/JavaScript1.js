function solve(args) {
    ...
    var pow = new BigNumber(1);
    for (var i = inputNumber.length - 1; i >= 0 ; i--, pow = pow.multiply(168)) {
        if (i === 0 || !isLower(inputNumber.charAt(i - 1))) {
            result = result.add(pow.multiply(inputNumber.charCodeAt(i) - 'A'.charCodeAt(0)));
        } else {
            result = result.add(pow.multiply((inputNumber.charCodeAt(i - 1) - 'a'.charCodeAt(0) + 1) * 26 + inputNumber.charCodeAt(i) - 'A'.charCodeAt(0)));
            i--;
        }
    }
