function solve(args) {
    ...
    var numbers = ["-!", "**", "!!!", "&&", "&-", "!-", "*!!!", "&*!", "!!**!-"],
        ...
        pow = new BigNumber(1),
        currIndex = -1,
        numberResult = new BigNumber(0);

    for (i = 0; i < args[0].length; i++) {

        temp += args[0].charAt(i);
        currIndex = numbers.indexOf(temp);
    ...
    for (i = result.length - 1; i >= 0; i--, pow = pow.multiply(9)) {
         numberResult = numberResult.add(new BigNumber(result.charAt(i)).multiply(pow));
    }

    return numberResult.valueOf();
}
